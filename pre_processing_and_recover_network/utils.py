...
def calculate_loss(y_label, logits):
    ...
    all_loss = []
    for i in range(len(transform_task)):
        loss = tf.reduce_mean(input_tensor=tf.compat.v1.losses.sigmoid_cross_entropy(multi_class_labels=y_label[i], logits=logits[i]))
...
def write_summary(loss, total_loss, f1_score, epoch_counter, isTraining, summary_writer):
    ...
    for i in range(len(task_name)):
        t = task_name[i]
        if isTraining:
            summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag="train loss/"+ t,          simple_value        =   loss[i][0])])
            summary_writer.add_summary(summary, epoch_counter)
            summary_writer.flush()

            summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag="train F1 score/"+ t,      simple_value        =   f1_score[0][i])])
            summary_writer.add_summary(summary, epoch_counter)
            summary_writer.flush()
            
        else:
            summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag="test loss/"+ t,           simple_value        =   loss[i][0])])
            summary_writer.add_summary(summary, epoch_counter)
            summary_writer.flush()

            summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag="test F1 score/"+ t,       simple_value        =   f1_score[0][i])])
            summary_writer.add_summary(summary, epoch_counter)
            summary_writer.flush()
            
    if isTraining:
        summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag="train loss/total_loss",          simple_value        =   total_loss)])
        summary_writer.add_summary(summary, epoch_counter)
        summary_writer.flush()        
    else:
        summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag="test loss/total_loss",           simple_value        =   total_loss)])
        summary_writer.add_summary(summary, epoch_counter)
        summary_writer.flush()
...
def model_result_store(y, y_pred, result_store, kfold):
    ...
    if((np.isnan(accuracy))or(np.isnan(precision))or(np.isnan(recall))or(np.isnan(f1_score))):
        print("we have nan")
    with open(result_store, 'a', newline='') as csvfile:
...
def one_hot_encoding(arr, tr_index, te_index):
    ...
    num_of_class = len(np.unique(arr))
    min_val      = np.min(arr)rch import nn
...
import pa
    arr          = arr - min_val
    # We: num_of_class doesn't fit, there is bigger  value in arr (even after normaliztion) than num_of_class
    # we try to use None (in fact it's max_val+1)
    arr_int = arr.astype(int)
    max_val_ = (np.max(arr.astype(int)))
    #num_of_class = max_val_+1
    tr_encoded_array = tf.keras.utils.to_categorical(arr[tr_index], num_classes = num_of_class) #origonal code
    te_encoded_array = tf.keras.utils.to_categorical(arr[te_index], num_classes = num_of_class) 
    return tr_encoded_array, te_encoded_array
'''
